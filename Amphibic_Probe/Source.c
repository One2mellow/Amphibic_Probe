...
poolList_t* Pools(pixmat** mtrx, image_t image, poolList_t* pools, int width_flag); //Creating list of pools which contain size and center co. for each pool
...
int main() {
	...
	while (choice != 9)
	{
		...
		{
		case 1:
			pools = Pools(matrix, image, pools, width_flag);
			if (pools == NULL) {
				printf_s("\nTotal of 0 pools.\n");
				menu();
				break;
...
poolList_t* Pools(pixmat** mtrx, image_t image, poolList_t* pools, int width_flag) {
	int i, j;
	int** temp;
	int size;
	pix_t* root = NULL;
